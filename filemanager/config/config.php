...
$rootdir = str_replace('/filemanager/dialog.php', '', $_SERVER['PHP_SELF']);
define('DOCROOT',$docroot);
define('ROOTDIR',$rootdir);
define('SERVER',$_SERVER['SERVER_ADDR'].ROOTDIR);	

//$a = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] && ! in_array(strtolower($_SERVER['HTTPS']), array( 'off', 'no' ))) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'];
//die(ROOTDIR);

if (session_id() == '') session_start();

if (!file_exists('../files/source/')) {
    mkdir('../files/source/', 0777, true);
}
...
$config = array(
	...
	*/
	...
	'base_url' => ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] && ! in_array(strtolower($_SERVER['HTTPS']), array( 'off', 'no' ))) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'].ROOTDIR,
	/*
	|--------------------------------------------------------------------------
	...
	|
	*/
	'upload_dir' => '/files/source/'.$folder_bidang,
	/*
	|--------------------------------------------------------------------------
	...
	|
	*/
	'current_path' => '../files/source/'.$folder_bidang,
	...
	|
	*/
	'thumbs_base_path' => '../files/thumbs/'.$folder_bidang,
